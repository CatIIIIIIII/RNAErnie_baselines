...
class TransformerLayer(nn.Module):
    ...
    def build_self_attention(self):
        if self.attention_type == "standard":
            ...
            raise ValueError(
                f"Unrecognized attention type {self.attention_type}")
...
class PerformerAttention(MultiheadAttention):
    def __init__(
        ...
        self._attn_fn = FastAttention(
            dim_heads=self.head_dim, nb_features=num_features)
    ...
    def forward(
        ...
        q, k, v = map(
            lambda t: rearrange(t, "t b (h d) -> b h t d",
