...
class MSA:
    ...
    def hhfilter(
        ...
        # dir="/userhome/zhouyq/gaozhq/zhyk/projects/emb-transformer/data"
        with tempfile.TemporaryDirectory(dir=current_directory) as tempdirname:
            tempdir = Path(tempdirname)
            fasta_file = tempdir / "input.fasta"
            fasta_file.write_text(
                "\n".join(f">{i}\n{seq}" for i,
                          seq in enumerate(self.sequences))
            )
    ...
    def select_diverse(self, num_seqs: int, method: str = "hhfilter") -> "MSA":
        assert method in ("hhfilter", "sample-pretrained",
                          "diversity-max", "diversity-min")
    ...
    def invcov(self) -> np.ndarray:
        ...
        dtype = self.dtype
        self.dtype = np.uint8
        Y = OneHotEncoder(drop=[self.gap]).fit_transform(
            self.array.reshape(-1, 1)).toarray().reshape(self.depth, self.seqlen, -1)
        K = Y.shape[-1]
    ...
    def array(self) -> np.ndarray:
        if not hasattr(self, "_array"):
            self._array = np.array([list(seq)
                                   for seq in self.sequences], dtype="|S1")
        return self._array
        ...
        for record in SeqIO.parse(stofile, "stockholm"):
            ...
            if not keep_insertions:
                if valid_indices is None:
                    valid_indices = [
                        i for i, aa in enumerate(sequence) if aa != "-"]
        ...
        **kwargs,
        ...
        for record in SeqIO.parse(fasfile, "fasta"):
            ...
            if remove_lowercase_cols:
                if valid_indices is None:
                    valid_indices = [i for i, aa in enumerate(
                        sequence) if aa.isupper()]
    ...
    def write(self, outfile: PathLike, form: str = "fasta") -> None:
        SeqIO.write(
            (SeqIO.SeqRecord(Seq(seq), id=header, description="")
