...
import numba
from RNAMSM.utils.tensor import collate_tensors, numpy_seed
from RNAMSM.utils.typed import PathLike
from RNAMSM.utils.align import MSA
from RNAMSM.utils.tokenization import Vocab
    ...
    def __init__(
        ...
        with numpy_seed(seed):
            indices = np.random.permutation(
                np.arange(len(dataset)))  # type: ignore
            start, end = (
...
class NPZDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        if split_files is None:
            ...
            file_list = [f for f in file_glob if f.stem.split('.')[
                0] in split_files]
...
class A3MDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        assert sample_method in (
            "hhfilter", "sample-pretrained", "diversity-max", "diversity-min")
        data_file = Path(data_file)
        if not data_file.exists():
            raise FileNotFoundError(data_file)
        if not data_file.is_dir():
...
class TorchDataset(CollatableVocabDataset):
    def __init__(self, data_file: PathLike, vocab: Vocab):
        ...
        self.offsets, self.sizes = np.load(
            data_file.with_suffix(".fasta.idx.npy"))
...
BatchOrSequence = TypeVar(
    "BatchOrSequence", MaxTokenBatch, Sequence[MaxTokenBatch])
...
class AutoBatchingDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        indices = indices[chunk_size *
                          worker_rank: chunk_size * (worker_rank + 1)]
...
class RandomCropDataset(BaseWrapperDataset):
    def __init__(self, dataset: CollatableVocabDataset, max_seqlen: int):
        ...
        self.num_special = int(self.vocab.prepend_bos) + \
            int(self.vocab.append_eos)
    ...
    def __getitem__(self, idx):
        ...
        if max_alignments < num_alignments:
            indices = np.random.randint(
                1, num_alignments, size=max_alignments - 1)
...
class MaskedTokenWrapperDataset(BaseWrapperDataset):
    ...
    def __getitem__(self, idx):
        ...
        random_probs[(item == self.vocab.bos_idx) | (
            item == self.vocab.eos_idx)] = 1   # <cls>和<eos>的特殊符号不会被mask
        do_mask = random_probs < self.mask_prob  # 小于0.15为True

        # 生成label，没有被mask的为1，mask的显示为真实的idx
        tgt = item.masked_fill(~do_mask, self.vocab.pad_idx)
        mask_with_token = random_probs < (
            # 替换为mask token 小于0.15*0.9为True
            self.mask_prob * (1 - self.leave_unmasked_prob)
        )
        src = item.masked_fill(
            mask_with_token, self.vocab.mask_idx)  # 用mask_idx(11)去遮盖
        mask_with_random = random_probs < (
            self.mask_prob * self.random_token_prob)  # 随机替换为其他token 小于0.15*0.1为True
