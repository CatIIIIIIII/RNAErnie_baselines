...
def doSavePredict_single(_id, seq, predict_rsa, fout, des, pred_asa=None):
    ...
    temp = np.vstack((np.char.mod('%d', col1), col2, np.char.mod(
        '%.2f', col3), np.char.mod('%.3f', col4))).T
    if fout:
        np.savetxt(fout + f'{_id}.txt', (temp), delimiter='\t\t', fmt="%s",
                   header=f'#{des}\n#index\t\tnt\t\tASA\t\tRSA\n',
                   comments='')
...
if __name__ == '__main__':
    ...
    fstatic = os.path.join(
        args.rootdir, 'models/OH+RNA-MSM_Emb/statistic_dict_{}.pickle')
    device = args.device
    ...
    models = [(model_path, torch.load(model_path, map_location=torch.device(device))) for model_path in
              glob.glob(fmodel)]
    print(bcolors.BLUE,
          f'loading {len(models)} models from {fmodel} pattern', bcolors.RESET)
