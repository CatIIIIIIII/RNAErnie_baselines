...
class DataConfig:
    ...
    model_path: str = str(current_directory /
                          "pretrained/RNA_MSM.ckpt")
    num_workers: int = 3
...
def extract_feat(cfg: Config) -> None:
    ...
    with torch.no_grad():
        for rna_id, tokens in tqdm(rna_data):
            ...
            results = model(tokens.to(device), repr_layers=[
                            10], need_head_weights=True)
            ...
            # extract embedding
            embedding = results["representations"][0]
            start_idx = int(vocab.prepend_bos)
            end_idx = embedding.size(-2) - int(vocab.append_eos)
            embedding = embedding[:, 0, start_idx:end_idx,
                                  :].squeeze(0).cpu().numpy()
