...
class Baseline(nn.Module):
    def __init__(self,
                 ...
                 backbone_frozen=0,
                 # because most of models used in this pipeline are pretrained
        ...
        # 1.Build backbone
        self.backbone, self.backbone_alphabet = choose_backbone(
            self.backbone_name)
        self.backbone.lm_head.requires_grad_(False)

        self.backbone_control_info = {
            "return_contacts": False, "need_head_weights": False, "repr_layers": []}
        ...
        if self.pairwise_predictor_name != "none":
            for pp_name in self.pairwise_predictor_name.split(" "):
                ...
                self.update_backbone_contral_info(
                    self.downstream_modules[pp_name].input_type)
        ...
        for key in self.downstream_modules.keys():
            ...
            self.downstream_modules[key].apply(
                weights_init_classifier)   # 最优 reduction这块还挺重要的
    ...
    def forward(self, data):
        ...
        for key in self.downstream_modules.keys():
            ...
            ds_module_input = self.fetch_ds_module_input(
                data, results, self.downstream_modules[key].input_type)
            results[target_name] = self.downstream_modules[key](
                x, ds_module_input).float()    # .float() for fp16 to fp32
            ...
            if key == "token":
                ...
                if isinstance(ds_module_input_type[key], int):
                    ...
                    if layer_i not in self.backbone_control_info["repr_layers"]:
                        self.backbone_control_info["repr_layers"].append(
                            ds_module_input_type[key])
                ...
                if isinstance(results[key], dict):
                    ...
                    for sub_key in results[key].keys():
                        if key == "representations" and len(results[key][sub_key][i].shape) == 2:
                            ...
                            lrange = (0, self.backbone_alphabet.prepend_bos +
                                      data["length"][i] + self.backbone_alphabet.append_eos)
                            temp_results[key][sub_key] = results[key][sub_key][i][lrange[0]:lrange[1]].detach(
                            ).cpu()
                else:
        ...
        for key in self.backbone_frozen_output_cache[name_list[0]].keys():
            if isinstance(self.backbone_frozen_output_cache[name_list[0]][key], dict):
                ...
                for sub_key in self.backbone_frozen_output_cache[name_list[0]][key].keys():
                    temp_results = []
                    for i, descritption in enumerate(name_list):
                        temp_results.append(self.backbone_frozen_output_cache[descritption][key][sub_key].to(
                            data["token"].float()))
                    try:
                        results[key][sub_key] = torch.stack(temp_results, dim=0)
                    except:
                        results[key][sub_key] = self.stack_variable_length_tensors(
                            temp_results)
            else:
                temp_results = []
                for i, descritption in enumerate(name_list):
                    temp_results.append(
                        self.backbone_frozen_output_cache[descritption][key].to(data["token"].float()))
                try:
                    results[key] = torch.stack(temp_results, dim=0)
                except:
                    results[key] = self.stack_variable_length_tensors(
                        temp_results)
        return results
    ...
    def stack_variable_length_tensors(self, vl_tensors):
        ...
        dim_max_value = torch.Tensor(dim_value).max(dim=0)[
            0].int().numpy().tolist()
        ...
        for t in vl_tensors:
            ...
            pad_tensors.append(torch.nn.functional.pad(
                t, pad_shape, mode="constant"))
        ...
        if load_choice == "backbone":
            ...
            for key in param_dict.keys():
                ...
                try:
                    ...
                    print(
                        "Cannot load %s, Maybe you are using incorrect framework" % key)

            for i in map_dict.keys():
                self.state_dict()[map_dict[i]["tar"]].copy_(
                    param_dict[map_dict[i]["src"]])
