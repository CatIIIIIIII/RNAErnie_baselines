...
def load_hub_workaround(url, download_name=None):
    try:
        data = torch.hub.load_state_dict_from_url(
            url, progress=True, map_location='cpu', file_name=download_name)
    except RuntimeError:
...
def load_model_and_alphabet_core(model_data, regression_data=None, theme="protein"):
    ...
    alphabet = fm.Alphabet.from_architecture(
        model_data["args"].arch, theme=theme)
    ...
    if model_data["args"].arch == 'roberta_large':
        ...
        def prs2(s): return ''.join(s.split('sentence_encoder.')
...
def rna_fm_t12(model_location=None):
    if model_location is not None and os.path.exists(model_location):
        # local
        # "./pretrained/RNA-FM_pretrained.pth"
...
def load_rnafm_model_and_alphabet_hub(model_name, theme="rna"):
    if model_name == "rna_fm_t12":
        ...
        model_data = load_hub_workaround(
            url, download_name="RNA-FM_pretrained.pth")
