...
parser.add_argument('--maskrate', '-m', type=float, default=0.0,
                    help='mask rate')
parser.add_argument('--pretraining', '-pre', type=str,
                    help='use pretrained weight')
parser.add_argument('--outputweight', type=str, help='output path for weights')
parser.add_argument('--algorithm', type=str,
                    default="global", help='algorithm method')
parser.add_argument('--data_mlm', '-d', type=str,
                    nargs='*', help='data for mlm training')
parser.add_argument('--data_mul', type=str, nargs='*',
                    help='data for mul training')
parser.add_argument('--data_alignment', type=str, nargs='*',
                    help='data for alignment test')
parser.add_argument('--data_clustering', type=str,
                    nargs='*', help='data for clustering test')
parser.add_argument('--data_showbase', type=str, nargs='*',
                    help='data for base embedding')
parser.add_argument('--data_embedding', type=str, nargs='*',
                    help='data for base embedding')
parser.add_argument('--embedding_output', type=str, nargs='*',
                    help='output file for base embedding')
...
class TRAIN:
    ...
    def train_MLM_SFP(self, model, optimizer, dl_MLM_SFP, num_epochs, task_type):
        for epoch in range(num_epochs):
            ...
            for batch in dl_MLM_SFP:
                ...
                mlm_loss = torch.tensor(0.0) if torch.isnan(
                    mlm_loss) else mlm_loss
                ...
                if task_type == "SFP":
                    z0_list, z1_list = self.module.em(
                        encoded_layers0, seq_len_0), self.module.em(encoded_layers1, seq_len_1)
                    sfp_loss, sfp_correct = self.module.train_SFP(
                        low_seq_0, seq_len_0, low_seq_1, seq_len_1, family_0, family_1, z0_list, z1_list)
                    sfp_loss = torch.tensor(0.0) if torch.isnan(
                        sfp_loss) else sfp_loss
                ...
                if task_type == "MUL":
                    ...
                    mul_loss = self.module.train_MUL(
                        z0_list, z1_list, common_index_0, common_index_1, seq_len_0, seq_len_1)
                    mul_loss = torch.tensor(0.0) if torch.isnan(
                        mul_loss) else mul_loss
            ...
            print('Epoch {}/{} | MLM Loss: {:.4f} MLM Acc: {:.4f}| SFP Loss: {:.4f} SFP Acc: {:.4f}| MUL Loss: {:.4f}| time: {:.4f} sec.'.format(
                epoch+1, num_epochs, epoch_mlm_loss, epoch_mlm_correct, epoch_sfp_loss, epoch_sfp_correct, epoch_mul_loss, time.time() - t_epoch_start))
            t_epoch_start = time.time()
        if args.outputweight:
            torch.save(model.state_dict(), args.outputweight +
                       '{0:%m_%d_%H_%M}'.format(current_time))
            torch.save(model.state_dict(), args.outputweight)
        return model
    ...
    def align(self, model, dl):
        ...
        for batch in dl:
            ...
            len_TP, len_pred_match, len_ref_match = self.module.test_align(
                low_seq_0, low_seq_1, z0_list, z1_list, common_index_0, common_index_1, seq_len_0, seq_len_1, args.show_aln)
            TP += len_TP
    ...
    def test(self, ds, test_loader, model):
        ...
        for i in range(data_num):
            single_seq = MyDataset("CLU", np.tile(ds.low_seq[i], (data_num, 1)), np.tile(
                ds.low_seq[i], (data_num, 1)), np.tile(ds.family[i], (data_num, 1)), np.tile(ds.seq_len[i], data_num))
            ...
            for data0, data1 in zip(test_loader, single_seq):
                ...
                low.append(torch.squeeze(logits).to(
...
def objective():
    ...
    if args.pretraining:
        model.load_state_dict(torch.load(
            args.pretraining, map_location='cpu'), strict=False)
