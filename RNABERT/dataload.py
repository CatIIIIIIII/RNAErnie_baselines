...
class DATA:
    ...
    def load_data_MLM_SFP(self, data_sets):
        ...
        gapped_seqs = np.tile(onehot_seq(
            gapped_seqs, self.max_length*5), (self.mag, 1))
        ...
        ds_MLM_SFP = MyDataset("MLM", low_seq, masked_seq, family,
                               seqs_len, low_seq_1, masked_seq_1, family_1, seqs_len_1)
    ...
    def load_data_EMB(self, data_sets):
        ...
        ds_MLM_SFP_ALIGN = MyDataset(
            "SHOW", low_seq, masked_seq, family, seqs_len)
    ...
    def load_data_MUL(self, data_sets, train_type):
        ...
        low_seq_1, masked_seq_1, family_1, seqs_len_1, common_index, common_index_1 = self.sfp_data(
            low_seq, masked_seq, family, seqs_len, family_ratio=0.0, gapped_seqs=gapped_seqs)
        ds_MLM_SFP_ALIGN = MyDataset("MUL", low_seq, masked_seq, family, seqs_len,
                                     low_seq_1, masked_seq_1, family_1, seqs_len_1, common_index, common_index_1)
        dl_MLM_SFP_ALIGN = torch.utils.data.DataLoader(
    ...
    def load_data_SSL(self, data_sets):
        ...
        for i, data_set in enumerate(data_sets):
            ...
            for j, record in enumerate(align):
                ...
                seq = gapped_seq.replace('-', '').replace('.', '')
                ss = ''.join([cons_SS[i]
                             for i, s in enumerate(list(gapped_seq)) if s != "-"])
                if set(seq) <= set(['A', 'T', 'G', 'C', 'U']) and len(list(seq)) < self.max_length:
                    seqs.append(seq)
                    families.append(i)
                    gapped_seqs.append(gapped_seq)
        ...
        transform = transforms.Compose([transforms.ToTensor()])
        low_seq_1, masked_seq_1, family_1, seqs_len_1, common_index, common_index_1, SS_1 = self.sfp_data(
            low_seq, masked_seq, family, seqs_len, family_ratio=0.0, gapped_seqs=gapped_seqs, SS=SS)
        ds_MLM_SFP_ALIGN = MyDataset("SSL", low_seq, masked_seq, family, seqs_len, low_seq_1,
                                     masked_seq_1, family_1, seqs_len_1, common_index, common_index_1, SS, SS_1)
        dl_MLM_SFP_ALIGN = torch.utils.data.DataLoader(
            ds_MLM_SFP_ALIGN, self.batch_size, shuffle=True)
        return dl_MLM_SFP_ALIGN
    ...
    def load_data_SHOW(self, data_sets):
        ...
        gapped_seqs = np.tile(onehot_seq(
            gapped_seqs, self.max_length*5), (self.mag, 1))
        # SS = np.tile(secondary_num(SS, self.max_length), (self.mag, 1))
        SS = list(itertools.chain.from_iterable([list(fgb.BulgeGraph.from_dotbracket(
            db).to_element_string().ljust(440, 'X')) for db in SS]))
        family = np.tile(np.array(families), self.mag)
        seqs_len = np.tile(np.array([len(i) for i in seqs]), self.mag)
        k = 1
        kmer_seqs = kmer(seqs, k)
    ...
    def load_data_CLU(self, data_sets):
        ...
        dl_CLU = torch.utils.data.DataLoader(
            ds_CLU, self.batch_size, shuffle=False)
    ...
    def sfp_data(self, low_seq, masked_seq, family, seqs_len, family_ratio=0.5, gapped_seqs=None, SS=None):
        ...
        if gapped_seqs is not None:
            ...
            A1 = np.array([np.pad(i, ((0, self.max_length-len(i))))
                          for i in A1])
            B1 = [i[np.where(i != 0)]-1 for i in B1]
            B1 = np.array([np.pad(i, ((0, self.max_length-len(i))))
                          for i in B1])
...
def base_to_num(seq, pad_max_length):
    seq = [list(i.translate(str.maketrans(
        {'A': "2", 'U': "3", 'G': "4", 'C': "5"}))) for i in seq]
...
def num_to_base(seq):
    seq = seq.tolist()
    seq = ["".join(map(str, i)).replace('0', '').translate(
        str.maketrans({'2': "A", '3': "U", '4': "G", '5': "C"})) for i in seq]
    return seq
...
def onehot_seq(gapped_seq, pad_max_length):
    gapped_seq = [list(i.translate(str.maketrans(
        {'-': "0", '.': "0", 'A': "1", 'U': "1", 'G': "1", 'C': "1"}))) for i in gapped_seq]
...
def secondary_num(SS, pad_max_length):
    SS = [list(i.translate(str.maketrans({'.': "0", ':': "1", '<': "2", '>': "2", '(': "3", ')': "3", '{': "3", '}': "3",
               '[': "3", ']': "3", 'A': "4", 'a': "4", 'B': "4", 'b': "4", '-': "5", '_': "6", ',': "7"}))) for i in SS]
