...
def show_base_PCA(features, targets, substructure):
    ...
    # ssdict_ref = {"s":"base pair in stem", "i":"Bulges and interior loop", "h":"hairpin loop", "m":"multiloop", "f":"external
    # loop(5)", "t":"external loop(3)"}
    ssdict_ref = {"s": "base pair in stem",
                  "h": "hairpin loop", "t": "external loop"}
    for i, ssub in enumerate(substructure):
        if ssub == "i" or ssub == "m" or ssub == "f":
            substructure[i] = "h"

    # ssdict_ref = {0:"unknown or pad", 1: "external loop", 2:"basepairs in simple stem loops", 3:"basepairs enclosing
    # multifurcations",
    # 4:"pseudoknot", 5:"Bulges and interior loops", 6:"Hairpin loops", 7:"Multibranch loops"}
    # ssdict_ref = {0:"unknown or pad", 1: "external loop", 2:"stacking", 3:"basepairs enclosing multifurcation",
    # 4:"pseudoknot",
    # 5:"Bulges and interior loop", 6:"Hairpin loop", 7:"Multibranch loop"}
    ...
    plt.show()
    plt.savefig("../png/{}_result_{}_base.png".format(x,
                int(time.time())), dpi=300)
    plt.close()
    ...
    plt.subplots_adjust(right=0.7)
    plt.show()
    plt.savefig("../png/{}_result_{}_ss_ALL.png".format(x,
                int(time.time())), dpi=300)
    plt.close()
    ...
    for label1 in np.unique(targets)[1:]:
        ...
        for label2 in list(set(substructure)):
            if not label2 == 'X':
                index = [i for i, x in enumerate(
                    list(np.array(substructure)[targets == label1])) if x == label2]
                plt.scatter(transformed[index, 0],
        ...
        plt.subplots_adjust(right=0.7)
        plt.show()
        plt.savefig("../png/{}_result_{}_ss_{}.png".format(x,
                    int(time.time()), basedict[label1]), dpi=300)
        plt.close()
